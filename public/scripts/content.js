...
async function extractJobData(card) {
  try {
    ...
    // Basic location restriction defaults
    const locationRestrictions = {
      hasRestrictions: false,
      restrictionTypes: [],
      internationalFriendly: true,
      confidence: 0,
      summary: 'No location restrictions detected'
    };
    ...
    return {
      ...
      // NEW: Location restriction fields
      hasLocationRestrictions: locationRestrictions.hasRestrictions,
      locationRestrictionTypes: locationRestrictions.restrictionTypes.join(', '),
      internationalFriendly: locationRestrictions.internationalFriendly,
      locationRestrictionConfidence: locationRestrictions.confidence,
      locationRestrictionSummary: locationRestrictions.summary,
      timestamp: new Date().toISOString(),
    };
  } catch (error) {
...
function downloadAsCSV(jobs, headers) {
  ...
  let updatedHeaders = [
    ...
    "Local Only", 
    "Requires Visa",
    "Has Location Restrictions",
    "Location Restriction Types",
    "International Friendly",
    "Location Restriction Confidence",
    "Location Restriction Summary",
    "Legacy Keywords",
    "Keyword Count"
  ];
  ...
  const csvContent = [
    ...
    ...jobs.map((job) => {
      let row = [
        ...
        job.hasLocationRestrictions ? "Yes" : "No",
        `"${escapeCsv(job.locationRestrictionTypes || '')}"`,
        job.internationalFriendly ? "Yes" : "No",
        job.locationRestrictionConfidence || 0,
        `"${escapeCsv(job.locationRestrictionSummary || '')}"`,
